...
const Search = ({ params }) => {
  ...
  const { data, isLoading } = useSWR(
    `https://api.themoviedb.org/3/search/movie?api_key=${process.env.NEXT_PUBLIC_API_KEY}&query=${id}&page=${page}`,
    fetcher
  );
  ...
  const renderPaginationButtons = () => {
    ...
    //     1,
    //     0,
    //     <span key="dots-start" className="text-white mx-1">
    //       ...
    //     </span>
    //   );
    // }
  ...
  return (
    data && (
      <div className="max-w-full lg:m-20 md:m-15 m-10">
        <h2 className="text-2xl font-semibold ">Search for "{id}"</h2>
        <h2 className="text-2xl font-semibold pt-10">Movies result</h2>
